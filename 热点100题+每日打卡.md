...
+ 动态规划；
+ `dp[i][j]`表示`p`的前`i`个字符和`s`的前`j`个字符是否匹配，`dp[0][0]`为`true`表示空字符串匹配；
+ 当`s`的第`j`个字符和`p`的第`i`个字符匹配或者为`？`时，`dp[i][j] = dp[i - 1][j - 1]`；
+ 当`p`的第`i`个字符为`*`时，`dp[i][j] = dp[i - 1][j] | dp[i][j - 1]`；
+ 其中`dp[i - 1][j]`为`*`不匹配字符，`dp[i][j - 1]`为`*`匹配了当前字符。
